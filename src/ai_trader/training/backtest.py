...
def run_strategy_backtest(
    ...
    candidate_rules = tuple(rules or default_strategy_rules())
    ...
    for rule in candidate_rules:
        ...
        if _is_eligible(sort_metrics, config):
            leaderboard.append(evaluation)

    leaderboard.sort(
        key=lambda evaluation: (
            evaluation.train_metrics or evaluation.metrics
        ).score,
        reverse=True,
    )
...
def default_strategy_rules() -> tuple[StrategyRule, ...]:
...
def _view(example: LocalTrainingExample) -> _ExampleView:
    ticker = str(
        example.metadata.get("ticker")
        or example.signal_bundle.ticker
        or example.trade_plan.ticker
    ).upper()
...
def _matches(rule: StrategyRule, view: _ExampleView) -> bool:
    if view.direction is not rule.direction:
        return False
    if view.conviction < rule.min_conviction:
        return False
...
def _is_eligible(metrics: StrategyMetrics, config: StrategyBacktestConfig) -> bool:
    return metrics.trades >= config.min_trades and metrics.active_months >= config.min_active_months
...
def _rule_name(
    *,
    ...
    return f"{signal}_{direction.value}_c{min_conviction:.2f}_{fear_greed_range}{suffix}"
